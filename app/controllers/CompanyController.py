...
@router.post("/addDept")
def add_dept(payload:CreateDepartmentSchema,current_user=Depends(get_current_user),db: Session = Depends(get_db)):
    print("i am under")
...
def dept_list(page: int = 1,size: int = 10,current_user=Depends(get_current_user),db: Session = Depends(get_db),):
...
def update_company(deptId:int,payload:UpdateDeptSchema ,db: Session = Depends(get_db),current_user=Depends(get_current_user)):
    ...
    return update_dept_details(deptId=deptId, payload=payload,db=db,current_user=current_user)
...
def delete_company(deptId:int,db: Session = Depends(get_db),current_user=Depends(get_current_user)):
