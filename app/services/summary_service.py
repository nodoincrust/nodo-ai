...
def summarizeDocument(
    documentId: int, version: int, force_refine: bool = False
) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"=== Starting summarizeDocument for documentId={documentId}, version_id={version_id} ==="
        )
        ...
        if not ai_doc:
            logger.warning(
                f"AIDocument not found for document {documentId}, version {version_id}"
            )
            return {
                "status": "processing",
        ...
        if not chunks:
            ...
            if not chunks:
                ...
                return {
                    "status": "error",
                    "message": "Failed to create chunks after attempt",
                }

        if len(chunks) > SUMMARY_TOP_K:
            logger.info(
                f"Selecting top {SUMMARY_TOP_K} chunks from {len(chunks)} total"
            )
        ...
        logger.info(
            f"Prepared context: {len(document_context)} chars from {len(chunks)} chunks"
        )
        ...
        if not parsed["summary"]:
            ...
            return {
                "status": "error",
                "message": "Summary generation failed - no valid summary returned",
            }

        logger.info(
            f"Parsed summary length: {len(parsed['summary'])}, tags count: {len(parsed.get('tags', []))}"
        )
            ...
            try:
                ...
                if retrieved:
                    ...
                    if refinement_result.get("status") == "success":
                        refined_parsed = safeJsonParse(
                            refinement_result["data"]["answer"]
                        )
                        ...
                        if refined_parsed["summary"]:
                            ...
                            parsed["citations"] = (
                                refined_parsed["citations"] or citations
                            )
